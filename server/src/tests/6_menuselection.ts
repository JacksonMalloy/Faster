    ...
    it(`Create Menu Selections`, async () => {
      for (let i = 0; i < 1000; i++) {
        const variables = {
          ...
          itemId: faker.random.number({ max: 500, min: 1 }),
          name: faker.random.word(),
          valueAdd: faker.commerce.price(5, 1),
        }

        const result = await menuSelectionRepo.createMenuSelection(variables)

        expect(result.menuSelection.tenantId).not.toBeNull()
        expect(result.menuSelection.header).not.toBeNull()
      }
    })
  })
  ...
  describe('Create Menu Selections For Test Account', () => {
    it(`Create Menu Selections`, async () => {
      for (let i = 0; i < 100; i++) {
        const variables = {
          tenantId: 12,
          itemId: faker.random.number({ max: 40, min: 1 }),
          name: faker.random.word(),
          valueAdd: faker.commerce.price(5, 1),
        }
